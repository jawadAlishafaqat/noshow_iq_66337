...
def train(X, y, save: bool = True):
    ...
    database = db._get_db()
    database.training_runs.insert_one({
...
def predict(record: dict):
    ...
    rec_high = 'Send SMS reminder + confirmation call. Consider double-booking.'
    rec_med = 'Send SMS reminder 24h before.'
    rec_low = 'No action needed.'

    rec = rec_high if risk == 'HIGH' else rec_med if risk == 'MEDIUM' else rec_low
